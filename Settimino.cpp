...
int S7Client::ISOConnect()
{
	...
	if (TCPClient.write(ISO_CR, sizeof(ISO_CR))==sizeof(ISO_CR))
	{
...
{
	uint16_t Length;
	if (TCPClient.write(S7_PN, sizeof(S7_PN))==sizeof(S7_PN))
	{
		RecvISOPacket(&Length);
		if (LastError==0)                 
		{
...
int S7Client::ReadArea(int Area, uint16_t DBNumber, uint16_t Start, uint16_t Amount, void *ptrData)
{
    ...
    while ((TotElements>0) && (LastError==0))
    {
		...
		if (TCPClient.write(PDU.H, Size_RD)==Size_RD)
		{
			RecvISOPacket(&Length);
			if (LastError==0)
			{
...
int S7Client::WriteArea(int Area, uint16_t DBNumber, uint16_t Start, uint16_t Amount, void *ptrData)
{
    ...
    while ((TotElements>0) && (LastError==0))
    {
		...
		if (TCPClient.write(PDU.H, IsoSize)==IsoSize)
		{
			RecvISOPacket(&Length);
			if (LastError==0)
			{
...
int S7Client::GetDBSize(uint16_t DBNumber, uint16_t *Size)
{
	...
	*Size=0;
	...
	if (TCPClient.write(PDU.H, sizeof(S7_BI))==sizeof(S7_BI))
	{
		RecvISOPacket(&Length);
		if (LastError==0)
		{
...
int S7Client::PlcStop()
{
	...
	if (TCPClient.write(PDU.H, sizeof(S7_STOP))==sizeof(S7_STOP))
	{
		RecvISOPacket(&Length);
		if (LastError==0)
		{
...
int S7Client::PlcStart()
{
	...
	if (TCPClient.write(PDU.H, sizeof(S7_START))==sizeof(S7_START))
	{
		RecvISOPacket(&Length);
		if (LastError==0)
		{
...
int S7Client::GetPlcStatus(int *Status)
{
	...
	if (TCPClient.write(PDU.H, sizeof(S7_PLCGETS))==sizeof(S7_PLCGETS))
	{
		RecvISOPacket(&Length);
		if (LastError==0)
		{
...
int S7Client::IsoExchangeBuffer(uint16_t *Size)
{
	...
	if (TCPClient.write(PDU.H, int(Size))==*Size)
		RecvISOPacket(Size);
	else
		LastError = errTCPDataSend;
